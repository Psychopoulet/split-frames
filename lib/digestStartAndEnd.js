	...
	// start tag detected
		...
		// end tag detected
			...
			this.emit("fullFrame", null !== this._escapeWith ?
				this._removeEscapeTag(this._frame.slice(firstStartFoundAt, endAt + endSize)) :
				Buffer.from(this._frame.slice(firstStartFoundAt, endAt + endSize))
			);

			this.push(
				null !== this._escapeWith ?
					this._removeEscapeTag(this._frame.slice(firstStartFoundAt + startSize, endAt)) :
					Buffer.from(this._frame.slice(firstStartFoundAt + startSize, endAt))
			);
