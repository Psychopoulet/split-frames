...
class SplitFrames extends node_stream_1.Transform {
    ...
    _digestEndOnly(enc) {
        ...
        const firstEnd = this._searchFirstEnd(0);
        // no end tag detected
        if (!firstEnd) {
            this._digesting = false;
        }
    ...
    _digestStartAndEnd(enc) {
        ...
        // start tag detected
            ...
            const firstEnd = this._searchFirstEnd(firstStart.end);
    ...
    _searchFirstTag(tag, beginAt = 0) {
        ...
        // escaped
            ...
            && 1 === endAt - startAt // must be a number to be escaped
            && this._escaped.includes(this._frame[startAt])
            && this._frame[startAt - 1] === this._escapeWith) {
            // escaper not escaped
    ...
    _searchFirstEnd(startEndAt) {
        const firstTag = this._searchFirstTag(this._endWith, startEndAt);
        if (firstTag) {
